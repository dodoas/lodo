...
//
// $Id: table-operations.js,v 1.2 2004/11/07 17:55:09 thomasek Exp $
...
// was pressed.
TableOperations.prototype.buttonPress = function(editor, action_id) {
	this.editor = editor;
	...
	switch (action_id) {
		...
		clearRow(otr);
		tr.parentNode.insertBefore(otr, /under/.test(action_id) ? tr.nextSibling : tr);
		editor.forceRedraw();
		editor.focusEditor();
		break;
	    case "TO-row-delete":
		...
		for (var i = rows.length; --i >= 0;) {
			var tr = rows[i];
			var ref = tr.cells[index + (/after/.test(action_id) ? 1 : 0)];
			var otd = editor._doc.createElement("td");
			otd.innerHTML = mozbr;
			tr.insertBefore(otd, ref);
		}
		...
		var otd = editor._doc.createElement("td");
		otd.innerHTML = mozbr;
		tr.insertBefore(otd, /after/.test(action_id) ? td.nextSibling : td);
		editor.forceRedraw();
		editor.focusEditor();
		break;
	    case "TO-cell-delete":
	    ...
	    default:
		alert("Button [" + action_id + "] not yet implemented");
	}
};
