...
class framework_logic_regnskapsrapport {
    ...
    function data($LineID, $Period) {
        ...
        if($LineID) {
            ...
            $queryThisYear = "select sum(V.AmountIn) as sumin, sum(V.AmountOut) as sumout from voucher V, accountplan as A where V.AccountPlanID=A.AccountPlanID and $where  V.VoucherPeriod<='".$this->thisEndPeriod."' and V.VoucherPeriod>='".$this->thisStartPeriod."' and V.Active=1 and A.EnableReportShort=1 and A.ReportShort='" . $LineID . "' group by A.ReportShort";
            #print "$queryThisYear";
