...
****************************************************************************/
	...
	if ( $package->package['status'] > 0 )
	{
		...
		$sendPost = "enterpriseSystemId=" . $config->GetConfig($config->TYPE_FAGSYSTEMID ) . "&batchId=" . $batchId . "&participantId=" . $config->GetConfig( $config->TYPE_ORGNO ) . "&passord=" . $config->GetConfig( $config->TYPE_PASSWORD );
		...
		if ( $rs = $db->Query( $sqlStr ) )
		{
			if ( $row = $db->NextRow( $rs ) )
			{
				...
				$sendPost .= "&sendersReference=" . $batchId . "-" . $row['instance_id'] . "&parentReference=&completed=1&locked=0&skjema=" . urlencode( $xml['body'] );
			}
			$db->EndQuery($rs);
		}
...
<?php
		...
		if ( !strncmp($result,'Status: OK', strlen('Status: OK')) )
		{
			...
			$sqlStr = "UPDATE altinn_packet SET status=" . $package->PACKAGESTATUS_SENDING . ",ts_modified='" . time() . "' WHERE packet_id=" . $packageId;
			if (!$db->Query( $sqlStr ))
			{
