...
****************************************************************************/
    ...
    if ( !is_numeric( $packageId ) ){
        ...
        $packageId = $package->CreateNewPackage( $packettype, $year, $termintype, $terminItem );
        //print ("MVa anh packetid:".$packageId);

        $data=$package->getOridArray();
        $kommuneList=$data['D5950'];
        print_r ($kommuneList);

        foreach ($kommuneList as $key => $kommuneNavn) {
            print ("Kommunenavn:".$kommuneNavn ."<br>");
        }
    }//if
