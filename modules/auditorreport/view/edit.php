...
<form name="auditorreport" action="<? print $_lib['sess']->dispatch."t=auditorreport.edit&amp;PeriodYear=".$PeriodYear ?>" method="post">
...
while($report_line = $_lib['db']->db_fetch_object($report_lines))
{
    ...
    <tr class="voucher">
        ...
        <td class="number"><? print $_lib['form3']->text(array('table'=>"auditorreportline", 'field'=>'AuditAmount',  'pk'=>$report_line->AuditorReportLineID, 'value'=>$_lib['format']->Amount($report_line->AuditAmount),  'width'=>'11', 'class'=>'number')); ?></td>
...
<table>
    <tr>
        <td width="500" class="number"><? print $_lib['form3']->submit(array('name'=>'action_auditorreport_update', 'value'=>'Lagre siden (S)', 'accesskey'=>'S')) ?></td>
    </tr>
</table>
</form>
<form name="skriv_ut" action="<? print $_lib['sess']->dispatch ?>t=auditorreport.print&PeriodYear=<? print $PeriodYear ?>" method="post" target="_new">
  <table>
    <tr>
        <? print $_lib['form3']->Input(array('type'=>'submit', 'name'=>'action_auditorreport_print', 'value'=>'Utskrift')) ?>
    </tr>
  </table>
</form>
