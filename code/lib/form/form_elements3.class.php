...
#type=submit or hidden allows use of the name parameter
    ...
    function input($args)
    {
        ...
        if(isset($args['validation']) && $args['validation'] == 'Date' or $args['validation'] == 'Datetime')
        {
            ...
            $element .= "var $obj = new calendar1(document.querySelector(\"form[name='".$args['form_name']."']\").querySelector(\"input[name='".$name."'\"));\n";
...
###########################################################
    ...
    function accountplan_number_menu($args) {
        ...
        if(isset($args['onchange'])) {
            $onchange = "onchange=\"" . $args['onchange'] . "\"";
        } else {
            $onchange = "";
        }

        $element = "<select $onchange $disabled $selectedcolor name=\"".$name."\" tabindex=\"".$args['tabindex']."\" accesskey=\"".$args['accesskey']."\" class=\"".$args['class']."\" " . $element;
