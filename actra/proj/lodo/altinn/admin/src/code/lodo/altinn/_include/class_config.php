...
class Config
{
	...
	function GetConfig( $type )
	{
		...
		if ( $type == $this->TYPE_TERMIN )
		{
			...
			if ($_sess->get_company('VatPeriod') != '') {$retVal = $this->config['termintype'];}

			return($retVal);
		}
		elseif ( $type == $this->TYPE_ORGNO ) { return(str_replace(' ', '', $_sess->get_company('OrgNumber'))); }
		elseif ( $type == $this->TYPE_MVABANKACCOUNT )
		{
			$retVal = $this->config['mvabankaccount'];
			if ( !is_numeric($retVal) || $retVal == 0 )
			{
				$retVal = str_replace(' ', '', $_sess->get_company('BankAccount'));
			}
